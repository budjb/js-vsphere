...
/**
 * NOTE: This file is not our own and can be download from a VMware fling at:
 *
 *     https://flings.vmware.com/vsphere-sdk-for-javascript
 */
(function () {
  'use strict';
  ...
  function toCamelCase(str) {
    return formatName(str).replace(/[^.]+$/, function (match) {
      return /^[A-Z]{2}$/.test(match) ? match : match.charAt(0).toLowerCase() + match.substr(1);
    });
  }
  ...
  function request(options) {
    return new Promise(function (resolve, reject) {
      ...
      var target = protocol + '://' + options.hostname + (isUndefined(options.port) ? '' : ':' + options.port);
      options.headers = options.headers || {};
      options.headers['Content-Type'] = options.contentType;
      if (options.proxy) {
        options.headers[options.proxyHeader] = target;
      }
      var url = (options.proxy ? '' : target) + options.path + (isUndefined(options.query) ? '' : '?' + options.query);
  ...
  function convert(obj, key1, key2) {
    return keys(obj).reduce(function (previous, current) {
      ...
      if (isUndefined(arr)) {
        ...
        var arr2 = findTypeArr(getNamespaces(key2), getNamespaceName(key1, arr), getTypeName(key1, arr));
        var instance = createInstance(key2, arr2);
        previous[current] = merge(instance, convert(obj[current], key1, key2));
      }
      return previous;
    }, {});
  }
    ...
    if (isUndefined(metadata[key])) {
      ...
      each(obj.namespaces, function (namespace, index) {
        ...
        each(namespace.operations, function (operationName, operationIndex) {
          ...
          defineProperty(metadata[key].methods[namespace.name], toCamelCase(operationName), createMethod(key, arr));
        });
        ...
        each(namespace.types, function (typeName, typeIndex) {
          ...
          defineProperty(metadata[key].objects[namespace.name], toPascalCase(typeName), createObject(key, arr));
        });
        if (!isUndefined(namespace.services)) {
          ...
          each(namespace.services, function (serviceName, serviceIndex) {
            ...
            defineProperty(metadata[key].packages[namespace.name], toCamelCase(serviceName), createPackage(key, arr));
          });
        }
      });
    }
  ...
  function loadMetadata(hostname, options, key) {
    ...
    return implementation
      .loadDefinitions(hostname, options)
      .then(function (obj) {
  ...
  function lookupMetadata(hostname, options) {
    ...
    try {
      ...
      var methods = createMethods(key, hostname, options, new Set());
      var objects = getObjects(key);
      return options.init(key, options, methods, objects).then(function (result) {
        if (key !== result.key) {
        ...
        request({
          ...
          function (req) {
            if (req.status === 500 && req.responseText) {
              reject(implementation.deserializeError(key, implementation.parseResponse(req)));
            } else {
  ...
  function createMethods(key, hostname, options, handlers, props) {
    ...
    each(keys(getMethods(key)), function (namespace) {
      ...
      each(keys(getMethods(key)[namespace]), function (name) {
        methods[namespace][name] = getMethods(key)[namespace][name].bind(null, hostname, options, handlers, props);
      });
    });
    return methods;
  }
  ...
  function createObject(key, arr) {
    ...
    if (enumerations.length !== 0) {
      ...
      return getTypeConstants(key, arr).reduce(
  ...
  function createService(hostname, options) {
    return lookupMetadata(hostname, options).then(function (result) {
      ...
      var methods = createMethods(result.key, hostname, options, handlers, result.props);
      ...
      var service = merge(
        {
          ...
          serializeObject: implementation.serializeObject.bind(null, result.key),
          deserializeObject: implementation.deserializeObject.bind(null, result.key),
        },
        result.props,
      );
      ...
      union
        ...
        .forEach(function (namespace) {
          ...
          defineProperty(service, prefixes[namespace], merge(methodInterfaces, objectInterfaces, packageInterfaces));
        });
  ...
  function createExports(vsphere) {
    if (!isUndefinedType(typeof exports) && isObject(exports) && !isUndefinedType(typeof module) && isObject(module)) {
      ...
      switch (value.category) {
        ...
        case 'GENERIC':
          ...
          if (!isUndefined(instantiation.element_type)) {
            result.typeElement = parseType(namespaces, getValue(instantiation.element_type));
          }
          if (!isUndefined(instantiation.map_key_type)) {
            result.typeKey = parseType(namespaces, getValue(instantiation.map_key_type));
          }
          if (!isUndefined(instantiation.map_value_type)) {
            result.typeValue = parseType(namespaces, getValue(instantiation.map_value_type));
          }
          return result;
        case 'USER_DEFINED':
    ...
    function processDefinition(data) {
      var namespaces = [
        ...
        },
        {
          ...
          operations: [],
          services: [],
          types: ['LIST', 'MAP', 'OPTIONAL', 'SET'],
        },
      ];
      var operations = [[], []];
      var services = [[], []];
      var types = [[], []];
      var union = unite(keys(data.operations), keys(data.services), keys(data.types)).sort();
      ...
      each(union, function (namespace) {
        ...
        operations.push(
          operationKeys.map(function (name) {
      ...
      each(union, function (namespace) {
        ...
        services.push(
          serviceKeys.reduce(function (previous, current) {
      ...
      each(union, function (namespace) {
        ...
        types.push(
          typeKeys.reduce(function (previous, current) {
            ...
            if (!isUndefined(value.values)) {
              ...
              var container = findTypeArr(namespaces, namespace, value.container.name);
    ...
    function processServices(data, namespace, obj) {
      each(obj.services, function (service) {
        ...
        each(value.operations, function (operation) {
          ...
          data.operations[namespace][entry.key + '.' + value.key] = getValue(value.value);
        });
    ...
    var implementation = {
        ...
        return merge(
          ...
          defaults,
          {
            init: function (key, options, methods, objects) {
              ...
              var fingerprint =
                methods['com.vmware.vapi']['com.vmware' + '.vapi.metadata.metamodel.component.fingerprint'];
              return Promise.all(
                ...
                var version = values.reduce(function (previous, current) {
                  return previous + (isUndefined(current) ? '' : current.substr(0, 8));
                }, '');
                return {
      ...
      deserialize: function (key, arr, message) {
        if (!isUndefined(message.output)) {
          ...
          return implementation.deserializeObject(key, message.output, info);
      ...
      deserializeError: function (key, message) {
        ...
        err.info = implementation.deserializeObject(key, message.error, { type: type });
      ...
      deserializeObject: function (key, data, info) {
        ...
        if (implementation.isBuiltInType(arr)) {
          ...
          return data.map(function (obj) {
            return implementation.deserializeObject(key, obj, typeElement);
          });
        } else if (!isUndefined(typeElement)) {
          if (!isNull(data[name])) {
            return implementation.deserializeObject(key, data[name], typeElement);
          }
          ...
          return data.reduce(function (previous, current) {
            ...
            previous[entry.key] = implementation.deserializeObject(key, entry.value, typeValue);
            return previous;
          }, {});
        } else {
          var type = getType(key, arr);
          ...
          if (enumerations.length !== 0) {
            ...
            return attributes.reduce(function (previous, current) {
              if (!isNull(data)) {
                var value = implementation.deserializeObject(key, data[type.type][name][current.name], current);
      ...
      isArrayType: function (arr) {
        return getNamespaceIndex(arr) === 1 && (getNameIndex(arr) == 0 || getNameIndex(arr) == 3);
      },
      ...
      serialize: function (key, arr, args, props) {
        ...
        var input = args.reduce(function (previous, current, index) {
          ...
          previous[info.name] = implementation.serializeObject(key, current, getTypeName(key, info.type), info);
          return previous;
        }, {});
        return {
          ctx: merge(
      ...
      serializeObject: function (key, obj, name, info) {
        ...
        var typeKey = info.typeKey;
        var enumerations = getTypeEnumerations(key, arr);
        if (implementation.isBuiltInType(arr) || enumerations.length !== 0) {
          return obj;
        } else if (implementation.isArrayType(arr)) {
          return obj.map(function (item) {
            return implementation.serializeObject(key, item, getTypeName(key, typeElement.type), typeElement);
          });
        } else if (!isUndefined(typeElement)) {
          var typeResult = {};
          typeResult[getTypeName(key, arr)] = isUndefined(obj)
            ? null
            : implementation.serializeObject(key, obj, getTypeName(key, typeElement.type), typeElement);
          return typeResult;
        } else if (!isUndefined(typeKey)) {
          return keys(obj).map(function (objKey) {
            return {
              STRUCTURE: {
                'map-entry': {
                  key: objKey,
                  value: implementation.serializeObject(key, obj[objKey], objKey, info.typeValue),
                },
              },
            };
          });
          ...
          each(type.attributes, function (attr) {
            result[type.type][name][attr.name] = implementation.serializeObject(
              key,
              obj[formatName(attr.name)],
              attr.name,
              attr,
            );
          });
  ...
  function createSoapImplementation() {
    ...
    function createElementNS(namespace, name) {
      ...
      if (!isUndefined(firstChild.getAttribute) && !isNull(firstChild.getAttribute('lineNumber'))) {
    ...
    function toProperty(namespaces, node, refs) {
      ...
      each(node.attributes, function (attr) {
        ...
        if (name === 'ref') {
          ...
          var ref = refs[lookupNamespace(node, elementName[0])][elementName[1]];
    ...
    function getOperationElement(namespaces, data, node) {
      ...
      var message = data.messages[lookupNamespace(node, messageName[0])][messageName[1]];
      var part = getByNameNS(message, WSDL_NAMESPACE, 'part')[0];
      var elementName = getAttribute(part, 'element').split(':');
      return data.elements[lookupNamespace(node, elementName[0])][elementName[1]];
    }
        ...
        if (!isUndefined(cache[definition])) {
          ...
          request({
            ...
            function (req) {
              ...
              var links = slice(wsdslImports).concat(slice(xsImports)).concat(slice(xsIncludes));
              ...
              if (cache[definition].length === 0) {
                ...
                each(links, function (element) {
                  var location = getAttribute(element, 'location') || getAttribute(element, 'schemaLocation');
                  var url = parseUrl(location);
                  if (!isUndefined(url.scheme)) {
                    location = url.path + (url.query ? '?' + url.query : '');
                  } else if (url.path.indexOf('/') !== 0) {
                    location = definition.replace(/[^\/]+$/, location);
                  }
                  load(location, hostname, options, data, cache).then(
                    function () {
    ...
    function processResponse(doc, data) {
        ...
        each(node.childNodes, function (node) {
          switch (node.localName) {
            case 'binding':
              ...
              break;
            case 'message':
              data.messages[namespace][getAttribute(node, 'name')] = node;
              break;
            case 'portType':
              each(node.childNodes, function (node) {
                if (node.localName === 'operation') {
                  data.operations[namespace][getAttribute(node, 'name')] = node;
                }
              });
              break;
            case 'service':
      ...
      each(schemas, function (node) {
        ...
        data.attributeGroups[namespace] = data.attributeGroups[namespace] || {};
        each(node.childNodes, function (node) {
          switch (node.localName) {
            case 'attribute':
              data.attributes[namespace][getAttribute(node, 'name')] = node;
              break;
            case 'complexType':
            case 'simpleType':
              data.types[namespace][getAttribute(node, 'name')] = node;
              break;
            case 'element':
              ...
              each(node.childNodes, function (node) {
                if (node.localName === 'complexType' || node.localName === 'simpleType') {
                  ...
                  setAttribute(data.elements[namespace][name], 'type', node.lookupPrefix(namespace) + ':' + name);
                  data.types[namespace][name] = node;
                }
              });
              break;
            case 'attributeGroup':
              data.attributeGroups[namespace][getAttribute(node, 'name')] = node;
              break;
          }
        });
      });
    ...
    function getPortNamespace(data, namespace) {
      return lookupNamespace(data.ports[namespace], getAttribute(data.ports[namespace], 'binding').split(':')[0]);
    }
    ...
    function getPortOperations(data, namespace) {
      ...
      if (!isUndefined(data.ports[namespace])) {
        ...
        each(getByNameNS(binding, binding.namespaceURI, 'operation'), function (operation) {
    ...
    function processDefinition(data) {
      ...
      each(union, function (namespace) {
        types.push(
          keys(data.types[namespace] || {}).map(function (name) {
            var attributes = [];
            var elements = [];
            ...
            each(type.childNodes, function (node) {
              switch (node.localName) {
                ...
                case 'choice':
                case 'all':
                  maxOccurs = getAttribute(node, 'maxOccurs') || maxOccurs;
                  minOccurs = getAttribute(node, 'minOccurs') || minOccurs;
              }
            });
            each(getByNameNS(type, XS_NAMESPACE, 'element'), function (node) {
              ...
              if (node.parentNode.localName === 'choice') {
                el.minOccurs = '0';
                maxOccurs = getAttribute(node.parentNode, 'maxOccurs') || maxOccurs;
              }
              elements.push(el);
            });
            ...
            each(getByNameNS(type, XS_NAMESPACE, 'attribute'), function (node) {
              attributes.push(toProperty(namespaces, node, data.attributes));
            });
            each(getByNameNS(type, XS_NAMESPACE, 'anyAttribute'), function (node) {
              attributes.push(toProperty(namespaces, node));
            });
            each(getByNameNS(type, XS_NAMESPACE, 'attributeGroup'), function (node) {
              var parts = getAttribute(node, 'ref').split(':');
              var attrGroup = data.attributeGroups[lookupNamespace(node, parts[0])][parts[1]];
              each(getByNameNS(attrGroup, XS_NAMESPACE, 'attribute'), function (node) {
                attributes.push(toProperty(namespaces, node, data.attributes));
              });
              each(getByNameNS(attrGroup, XS_NAMESPACE, 'anyAttribute'), function (node) {
                attributes.push(toProperty(namespaces, node));
              });
            });
            var base;
            ...
            if (!isUndefined(extension)) {
              base = parseRef(namespaces, extension, getAttribute(extension, 'base'));
            }
            var enumerations = [];
            var unions = [];
            if (type.localName === 'simpleType') {
              each(getByNameNS(type, XS_NAMESPACE, 'restriction'), function (node) {
                each(getByNameNS(node, XS_NAMESPACE, 'enumeration'), function (enumeration) {
                  enumerations.push(getAttribute(enumeration, 'value'));
                });
              });
              each(getByNameNS(type, XS_NAMESPACE, 'union'), function (node) {
                each(getAttribute(node, 'memberTypes').split(/\s+/g), function (type) {
      ...
      each(union, function (namespace) {
        if (!isUndefined(data.ports[namespace])) {
          ...
          operations.push(
            portOperations.map(function (name) {
              var node = data.operations[portNamespace][name];
              var input = getOperationElement(namespaces, data, getByNameNS(node, WSDL_NAMESPACE, 'input')[0]);
              var output = getOperationElement(namespaces, data, getByNameNS(node, WSDL_NAMESPACE, 'output')[0]);
              return [
                {
                  action: getAttribute(data.actions[portNamespace][name], 'soapAction'),
                  name: getAttribute(input, 'name'),
                  type: parseRef(namespaces, input, getAttribute(input, 'type')),
                },
                {
                  type: parseRef(namespaces, output, getAttribute(output, 'type')),
                },
              ];
            }),
          );
    ...
    var implementation = {
      ...
      deserialize: function (key, arr, message) {
        var operation = getOperation(key, arr);
        var info = operation[OUTPUT_INDEX];
        var node = getFirstChild(getByNameNS(message, SOAP_NAMESPACE, 'Body')[0]);
      ...
      deserializeError: function (key, message) {
        ...
        if (!isUndefined(detail)) {
          ...
          var type = findTypeArr(getNamespaces(key), firstChild.namespaceURI, firstChild.nodeName);
          err.info = implementation.deserializeObject(key, firstChild, { type: type });
        }
        return err;
      },
      deserializeObject: function (key, data, info) {
          ...
          each(getTypeElements(key, arr), function (el) {
            ...
            each(data.childNodes, function (node) {
              if (node.localName === el.name) {
                var obj = implementation.deserializeObject(key, node, el);
                if (!isUndefined(obj)) {
                  if (unbounded) {
                    if (isArray(result)) {
                      result.push(obj);
            ...
            each(data.attributes, function (node) {
              if (node.name === el.name) {
                ...
                if (implementation.isBuiltInType(el.type)) {
                  result[el.name] = fromString(key, el.type, node.value);
      ...
      serialize: function (key, arr, args) {
        ...
        appendChild(appendChild(message, body), implementation.serializeObject(key, obj, info.name, info));
        return envelope;
      },
      serializeObject: function (key, obj, tag, info) {
        ...
        if (!isUndefined(info)) {
          ...
          if (
            getNamespaceIndex(objectArr) !== getNamespaceIndex(arr) ||
            getNameIndex(objectArr) !== getNameIndex(arr)
          ) {
            setAttributeNS(element, XSI_NAMESPACE, 'xsi:type', getTypeName(key, objectArr));
          }
        } else {
        ...
        each(getTypeElements(key, objectArr), function (el) {
          if (isUndefined(el.name)) {
            ...
            if (!((isUndefined(value) && props.indexOf(el.name) === -1) || isNull(value))) {
              var objects = isArray(value) ? value : [value];
              each(objects, function (elObj) {
                var child, namespaceName;
                if (!isObject(elObj) && implementation.isBuiltInType(el.type)) {
                  namespaceName = getNamespaceName(key, objectArr);
                  child = createElementNS(namespaceName, el.name);
                  appendChild(child, createTextNode(toString(key, el.type, elObj)));
                } else {
                  var enumerations = getTypeEnumerations(key, el.type);
                  if (enumerations.length !== 0) {
                    namespaceName = getNamespaceName(key, el.type);
                    child = createElementNS(namespaceName, el.name);
                    appendChild(child, createTextNode(elObj));
                  } else {
                    child = implementation.serializeObject(key, elObj, el.name, el);
                  }
                }
        ...
        each(getTypeAttributes(key, objectArr), function (attr) {
          if (isUndefined(attr.name)) {
            ...
            if (!((isUndefined(obj[attr.name]) && props.indexOf(attr.name) === -1) || isNull(obj[attr.name]))) {
              if (implementation.isBuiltInType(attr.type)) {
                setAttribute(element, attr.name, toString(key, attr.type, obj[attr.name]));
              } else {
        ...
        if (implementation.isBuiltInType(root)) {
          appendChild(element, createTextNode(toString(key, root, obj.value)));
        ...
        if (!isUndefined(any)) {
          each(props, function (prop) {
            ...
            each(objects, function (propObj) {
              appendChild(element, implementation.serializeObject(key, propObj, prop));
            });
          });
          props = [];
        }
        if (!isUndefined(anyAttribute)) {
          each(props, function (prop) {
            setAttribute(element, prop, toString(key, getObjectArr(key, obj[prop].constructor), obj[prop]));
          });
          props = [];
        }
        return element;
      ...
      validateObject: function (key, obj, tag, info) {
        ...
        if (!isUndefined(info)) {
          ...
          if (
            ...
            if (
              ...
              !(getTypeName(key, arr) === 'anyType' || obj instanceof getObject(key, arr))
            ) {
              ...
              each(objects, function (elObj) {
                if (!(!isObject(elObj) && implementation.isBuiltInType(el.type))) {
                  ...
                  if (enumerations.length !== 0) {
                    ...
                    implementation.validateObject(key, elObj, el.name, el);
                  }
                }
              });
            }
        ...
        if (!isUndefined(any)) {
          ...
          if (maxOccurs !== 'unbounded' && props.length > parseInt(maxOccurs)) {
    ...
    var xmlParser = new (isUndefinedType(typeof DOMParser) ? require('xmldom').DOMParser : DOMParser)();
    var xmlSerializer = new (isUndefinedType(typeof XMLSerializer) ? require('xmldom').XMLSerializer : XMLSerializer)();
    var xmlDocument = isUndefinedType(typeof document) ? xmlParser.parseFromString('<xml/>') : document;
  ...
  var storage = isUndefinedType(typeof localStorage)
    ? new (require('node-localstorage').LocalStorage)(require('path').join('.localStorage'))
    : localStorage;
  ...
  createExports({
    cisService: function (hostname, options) {
      return createService(
        hostname,
        jsonRpcImplementation.createOptions(
          {
            definitions: [
              'applmgmt',
              'com.vmware.cis',
    ...
    integrityService: function (hostname, options) {
      return createService(
        hostname,
        soapImplementation.createOptions(
          {
            init: function (key, options, methods, objects) {
              ...
              return integrityPort.retrieveVcIntegrityContent(serviceInstance).then(function (serviceContent) {
                return {
                  props: {
                    serviceContent: serviceContent,
                    serviceInstance: serviceInstance,
                  },
                  key: [options.key, serviceContent.about.apiVersion].join('/'),
                };
              });
            },
            endpoint: '/vci/sdk',
        ...
        soapImplementation.createOptions(
          {
            init: function (key, options, methods, objects) {
              ...
              return srmPort.retrieveContent(serviceInstance).then(function (serviceContent) {
                return {
                  props: {
                    serviceContent: serviceContent,
                    serviceInstance: serviceInstance,
                  },
                  key: [options.key, serviceContent.about.version].join('/'),
                };
              });
            },
        ...
        soapImplementation.createOptions(
          {
            ...
            prefixes: {
              ...
              wsta: 'http://www.rsa.com/names/2009/12/std-ext/' + 'WS-Trust1.4/advice',
        ...
        soapImplementation.createOptions(
          {
            init: function (key, options, methods, objects) {
              ...
              return vimPort.retrieveServiceContent(serviceInstance).then(function (serviceContent) {
                return {
                  props: {
                    serviceContent: serviceContent,
                    serviceInstance: serviceInstance,
                  },
                  key: [options.key + ':' + serviceContent.about.apiType, serviceContent.about.version].join('/'),
                };
              });
            },
